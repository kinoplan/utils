...
lazy val root = project.in(file(".")).aggregate(implicitsBoolean, implicitsCollection)
...
inThisBuild(List(
  ...
  scmInfo := Some(
    ScmInfo(url("https://github.com/kinoplan/utils"), "scm:git:git@github.com:kinoplan/utils.git")
  )
))

onChangedBuildSource in Global := ReloadOnSourceChanges

onLoad in Global := ("project root" :: (_: State)).compose((onLoad in Global).value)

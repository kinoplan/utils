  ...
  "DateTime" should {
    ...
    "return correct value writeTry" in
      assert(
        TestDataDateTime
          .handler
          .writeTry(TestDataDateTime(dateTime))
          .toOption
          .contains(document("value" -> bsonDateTime))
      )
    "return correct value readTry" in
      assert(
        TestDataDateTime
          .handler
          .readTry(document("value" -> bsonDateTime))
          .toOption
          .contains(TestDataDateTime(dateTime))
      )
  ...
  "LocalTime" should {
    ...
    "return correct value writeTry" in
      assert(
        TestDataLocalTime
          .handler
          .writeTry(TestDataLocalTime(localTime))
          .toOption
          .contains(document("value" -> bsonDateTime))
      )
    "return correct value readTry" in
      assert(
        TestDataLocalTime
          ...
          .readTry(document("value" -> bsonDateTime))
          .toOption
          .contains(TestDataLocalTime(localTime))
      )
    "return incorrect value readTry" in
      assert(
        TestDataLocalTime
  ...
  "LocalDate" should {
    ...
    "return correct value writeTry" in
      assert(
        TestDataLocalDate
          .handler
          .writeTry(TestDataLocalDate(localDate))
          .toOption
          .contains(document("value" -> bsonDateTime))
      )
    "return correct value readTry" in
      assert(
        TestDataLocalDate
          ...
          .readTry(document("value" -> bsonDateTime))
          .toOption
          .contains(TestDataLocalDate(localDate))
      )
    "return incorrect value readTry" in
      assert(
        TestDataLocalDate

...
class BsonAnyHandlersSpec extends AnyWordSpec {
  ...
  "BsonAnyHandlers#writeTry" should {
    "return correct value Int" in
      assert(TestData.handler.writeTry(TestData(1)).toOption.contains(document("value" -> 1)))
    "return correct value Long" in
      assert(TestData.handler.writeTry(TestData(1L)).toOption.contains(document("value" -> 1L)))
    "return correct value Double" in
      assert(TestData.handler.writeTry(TestData(1.1)).toOption.contains(document("value" -> 1.1)))
  ...
  "BsonAnyHandlers#readTry" should {
    "return correct value Int" in
      assert(TestData.handler.readTry(document("value" -> 1)).toOption.contains(TestData(1)))
    "return correct value Long" in
      assert(TestData.handler.readTry(document("value" -> 1L)).toOption.contains(TestData(1L)))
    "return correct value Double" in
      assert(TestData.handler.readTry(document("value" -> 1.1)).toOption.contains(TestData(1.1)))
    "return correct value String" in
      assert(TestData.handler.readTry(document("value" -> "test")).toOption.contains(TestData("test")))
    "return IllegalArgumentException" in
      assert(
        TestData
          .handler
          .readTry(document("value" -> BSONArray.empty))
          .failed
          .toOption
          .map(_.getMessage)
          .contains(

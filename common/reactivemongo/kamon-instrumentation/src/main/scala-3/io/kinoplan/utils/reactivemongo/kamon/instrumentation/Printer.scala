...
final case class Printer(ident: String = "  ", maxLength: Int) {
      ...
      doc
        ...
        .foreach {
          case BSONElement((name, value)) =>
            if (unsafePrint(name, value, 1, ident, f, sb)) boundary.break(true)
            if (append(sb, ",\n")) boundary.break(true)
          case _ => ()
        }
  ...
  private def unsafePrint(
    ...
    if (append(sb, currentIdent)) boundary.break(true)
    if (append(sb, f(name))) boundary.break(true)
    ...
    value match {
      ...
      case BSONArray(vs) =>
        if (vs.isEmpty) append(sb, "[]")
          ...
          if (append(sb, "[\n")) boundary.break(true)
          vs.foreach { v =>
            if (unsafePrint("", v, deep + 1, currentIdent + ident, f, sb)) boundary.break(true)
            if (append(sb, ",\n")) boundary.break(true)
          }
          sb.delete(sb.size - 2, sb.size)
          append(sb, s"\n$currentIdent]")
      ...
      case BSONDocument(elements) =>
        if (elements.isEmpty) append(sb, "{}")
          ...
          elements.foreach {
            case BSONElement((name, value)) =>
              if (unsafePrint(name, value, deep + 1, currentIdent + ident, f, sb))
                boundary.break(true)
              if (append(sb, ",\n")) boundary.break(true)
            case _ => ()
          }

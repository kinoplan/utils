...
final private[implicits] class BooleanOps(private val value: Boolean) extends AnyVal {
  ...
  @inline
  def fold[T](falseAction: => T)(trueAction: => T): T =
    if (value) trueAction
    else falseAction

  @inline
  def foldList[T](falseAction: => List[T]): List[T] =
    if (value) List.empty[T]
    else falseAction

  @inline
  def when[T](action: => T): Option[T] =
    if (value) Some(action)
    else None

  @inline
  def unless[T](action: => T): Option[T] =
    if (value) None
    else Some(action)

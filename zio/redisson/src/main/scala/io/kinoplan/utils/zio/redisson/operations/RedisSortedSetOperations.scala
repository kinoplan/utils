...
import io.kinoplan.utils.redisson.base.codec.{DefaultRedisEncoders, RedisDecoder, RedisEncoder}
...
trait RedisSortedSetOperations {
  ...
  def zDiff[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]]

  def zDiffStore[T: RedisDecoder](key: String, names: String*): Task[Int]

  def zDiffStore[T: RedisDecoder](key: String, names: Set[String]): Task[Int]
  ...
  def zInter[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]]
  ...
  def zInterStore(key: String, names: Set[String]): Task[Int]

  def zInterStore(key: String, aggregate: Aggregate, names: String*): Task[Int]

  def zInterStore(key: String, aggregate: Aggregate, names: Set[String]): Task[Int]
  ...
  def zUnion[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]]
  ...
  def zUnionStore(key: String, names: Set[String]): Task[Int]

  def zUnionStore(key: String, aggregate: Aggregate, names: String*): Task[Int]

  def zUnionStore(key: String, aggregate: Aggregate, names: Set[String]): Task[Int]
...
trait RedisSortedSetOperationsImpl extends RedisSortedSetOperations with DefaultRedisEncoders {
  ...
  override def bzmPopMax[T: RedisDecoder](
    ...
    count: Int,
    names: Set[String]
  ): Task[Map[String, Map[T, Double]]] = bzmPopMax(key, timeout, count, names.toSeq: _*)
  ...
  override def bzmPopMin[T: RedisDecoder](
    ...
    count: Int,
    names: Set[String]
  ): Task[Map[String, Map[T, Double]]] = bzmPopMin(key, timeout, count, names.toSeq: _*)
  ...
  override def zDiff[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]] = ZIO
    .fromCompletionStage(scoredSortedSet(key).readDiffAsync(names.toSeq: _*))
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zDiffStore[T: RedisDecoder](key: String, names: Set[String]): Task[Int] = ZIO
    .fromCompletionStage(scoredSortedSet(key).diffAsync(names.toSeq: _*))
    .map(_.intValue())
  ...
  override def zInter[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]] = ZIO
    .fromCompletionStage(scoredSortedSet(key).readIntersectionAsync(names.toSeq: _*))
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zInter[T: RedisDecoder](
    ...
    .fromCompletionStage(scoredSortedSet(key).readIntersectionAsync(aggregate, names.toSeq: _*))
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zInterStore(key: String, names: Set[String]): Task[Int] = ZIO
    .fromCompletionStage(scoredSortedSet(key).intersectionAsync(names.toSeq: _*))
    .map(_.intValue())
  ...
  override def zInterStore(key: String, aggregate: Aggregate, names: Set[String]): Task[Int] = ZIO
    .fromCompletionStage(scoredSortedSet(key).intersectionAsync(aggregate, names.toSeq: _*))
    .map(_.intValue())
  ...
  override def zmPopMax[T: RedisDecoder](
    ...
    count: Int,
    names: Set[String]
  ): Task[Map[String, Map[T, Double]]] = zmPopMax(key, timeout, count, names.toSeq: _*)
  ...
  override def zmPopMin[T: RedisDecoder](
    ...
    count: Int,
    names: Set[String]
  ): Task[Map[String, Map[T, Double]]] = zmPopMin(key, timeout, count, names.toSeq: _*)
  ...
  override def zRange[T: RedisDecoder](key: String, fromScore: Int, toScore: Int): Task[Iterable[T]] =
    ZIO
  ...
  override def zRangeByScore[T: RedisDecoder](
    ...
    .fromCompletionStage(
      scoredSortedSet(key)
        .valueRangeAsync(startScore, startScoreInclusive, endScore, endScoreInclusive)
    )
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zRangeByScore[T: RedisDecoder](
    ...
    .fromCompletionStage(
      scoredSortedSet(key)
        .valueRangeAsync(startScore, startScoreInclusive, endScore, endScoreInclusive, offset, count)
    )
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zRemRangeByLexMax(key: String, fromElement: String, fromInclusive: Boolean): Task[Int] =
    ZIO
  ...
  override def zUnion[T: RedisDecoder](key: String, names: Set[String]): Task[Iterable[T]] =
    zUnion(key, names.toSeq: _*)
  ...
  override def zUnion[T: RedisDecoder](
    ...
    aggregate: Aggregate,
    names: Set[String]
  ): Task[Iterable[T]] = zUnion(key, aggregate, names.toSeq: _*)
  ...
  override def zUnion[T: RedisDecoder](
    ...
    .fromCompletionStage(
      scoredSortedSet(key)
        .readUnionAsync(aggregate, JavaEncoders.encodeMapWithWeight(nameWithWeight))
    )
    .flatMap(JavaDecoders.decodeCollection(_))
  ...
  override def zUnionStore(key: String, names: Set[String]): Task[Int] =
    zUnionStore(key, names.toSeq: _*)
  ...
  override def zUnionStore(key: String, aggregate: Aggregate, names: Set[String]): Task[Int] =
    zUnionStore(key, aggregate, names.toSeq: _*)

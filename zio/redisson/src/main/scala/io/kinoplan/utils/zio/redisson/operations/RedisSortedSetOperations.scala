...
trait RedisSortedSetOperations {
  ...
  /** Intersect multiple sorted sets and return the result.
    ...
    */
  def zInter[T: RedisDecoder](key: String, args: SetIntersectionArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]]
  ...
  /** Intersect multiple sorted sets and return the result.
    ...
    *   The resulting iterable of the intersection.
    */
  def zInter[T: RedisDecoder](key: String, args: SetReadArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]]
  ...
  /** Add multiple sorted sets and return the union.
    ...
    */
  def zUnion[T: RedisDecoder](key: String, args: SetUnionArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]]
  ...
  /** Add multiple sorted sets and return the union.
    ...
    *   The resulting iterable of the union.
    */
  def zUnion[T: RedisDecoder](key: String, args: SetReadArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]]
...
trait RedisSortedSetOperationsImpl extends RedisSortedSetOperations with DefaultRedisEncoders {
  ...
  override def bzmPopMin[T: RedisDecoder](
    ...
    .fromCompletionStage(scoredSortedSet(key).pollFirstEntriesFromAnyAsync(timeout, count, keys: _*))
    .flatMap(JavaDecoders.fromMapScoredValue(_))
  ...
  override def zInter[T: RedisDecoder](key: String, args: SetIntersectionArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]] = ZIO
    .fromCompletionStage(scoredSortedSet(key).readIntersectionAsync(args))
    .flatMap(JavaDecoders.fromCollection(_))

  override def zInter[T: RedisDecoder](key: String, args: SetReadArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]] = zInter[T](key, args.asInstanceOf[SetIntersectionArgs])
  ...
  override def zUnion[T: RedisDecoder](key: String, args: SetUnionArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]] = ZIO
    .fromCompletionStage(scoredSortedSet(key).readUnionAsync(args))
    .flatMap(JavaDecoders.fromCollection(_))

  override def zUnion[T: RedisDecoder](key: String, args: SetReadArgs)(implicit
    codec: RCodec
  ): Task[Iterable[T]] = zUnion[T](key, args.asInstanceOf[SetUnionArgs])

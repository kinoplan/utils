...
trait RedisBitmapOperations {
  ...
  def bitOpAnd(key: String, names: Set[String]): Task[Unit]

  def bitOpOr(key: String, names: String*): Task[Unit]

  def bitOpOr(key: String, names: Set[String]): Task[Unit]

  def bitOpXor(key: String, names: String*): Task[Unit]

  def bitOpXor(key: String, names: Set[String]): Task[Unit]
  ...
  def setBit(key: String, indexes: Set[Long], value: Boolean): Task[Unit]
...
trait RedisBitmapOperationsImpl extends RedisBitmapOperations {
  ...
  override def bitOpAnd(key: String, names: Set[String]): Task[Unit] = bitOpAnd(key, names.toSeq: _*)

  override def bitOpOr(key: String, names: String*): Task[Unit] = ZIO
    .fromCompletionStage(bitSet(key).orAsync(names: _*))
    .unit

  override def bitOpOr(key: String, names: Set[String]): Task[Unit] = bitOpOr(key, names.toSeq: _*)

  override def bitOpXor(key: String, names: String*): Task[Unit] = ZIO
    .fromCompletionStage(bitSet(key).xorAsync(names: _*))
    .unit

  override def bitOpXor(key: String, names: Set[String]): Task[Unit] = bitOpXor(key, names.toSeq: _*)
  ...
  override def setBit(key: String, indexes: Set[Long], value: Boolean): Task[Unit] = ZIO

  ...
  def specs: Chunk[TestSpec[RedisClient, Throwable, TestResult]] = Chunk(
    ...
    TestSpec(
      ...
      for {
        ...
        case1 <- redis.zInter[String](key1, Seq(key2))
    ...
    TestSpec(
      ...
      for {
        ...
        case1 <- redis.zInterStore(key3, Seq(key1, key2))
        case2 <- redis.zRange[String](key3)
        case3 <- redis.zInterStore(key3, Map((key1, 2.0), (key2, 3.0)))
        case4 <- redis.zRange[String](key3)
        case5 <- redis.zInterStore(key3, Aggregate.MIN, Seq(key1, key2))
        case6 <- redis.zRange[String](key3)
        case7 <- redis.zInterStore(key3, Aggregate.MAX, Seq(key1, key2))
    ...
    TestSpec(
      ...
      for {
        ...
        _ <- redis.zAdd(key1, Map(("M", 5), ("N", 6), ("O", 7)))
        _ <- redis.zAdd(key2, Map(("N", 3), ("O", 2), ("P", 4)))
        case1 <- redis.zUnion[String](key1, Seq(key2))
        case2 <- redis.zUnion[String](key1, Aggregate.MIN, Seq(key2))
        case3 <- redis.zUnion[String](key1, Aggregate.MAX, Seq(key2))
      } yield assertTrue(
    ...
    TestSpec(
      ...
      for {
        ...
        _ <- redis.zAdd(key1, Map(("M", 5), ("N", 6), ("O", 7)))
        _ <- redis.zAdd(key2, Map(("N", 3), ("O", 2), ("P", 4)))
        case1 <- redis.zUnionStore(key3, Seq(key1, key2))
        case2 <- redis.zRange[String](key3)
        case3 <- redis.zUnionStore(key3, Map((key1, 2.0), (key2, 3.0)))
        case4 <- redis.zRange[String](key3)
        case5 <- redis.zUnionStore(key3, Aggregate.MIN, Seq(key1, key2))
        case6 <- redis.zRange[String](key3)
        case7 <- redis.zUnionStore(key3, Aggregate.MAX, Map((key1, 2.0), (key2, 3.0)))
        case8 <- redis.zRange[String](key3)
      } yield assertTrue(
        case1 == 4,

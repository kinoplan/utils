...
object MongoModule {
  ...
  def live(dbNames: NonEmptySet[String]): ZLayer[Any, Throwable, Map[String, ReactiveMongoApi]] =
    AsyncDriverResource.live ++ MongoConfig.live(dbNames.toSeq) >>>
      ZLayer.foreach(dbNames.toSeq)(ReactiveMongoApi.make).map(a => ZEnvironment(a.get.toMap))
